...
pub fn run(dir: &Path, source_id: &str, relation: &str, target_id: &str, remove: bool) -> Result<()> {
    ...
    if remove {
        println!("Unlinked {} {} {}", src_item.id, relation, tgt_item.id);
    } else {
        println!("Linked {} {} {}", src_item.id, relation, tgt_item.id);
    }
    Ok(())
}
