...
Each row is formatted as `icon id [Px] [phase] [type] title [tags] due:… [Nsp]`. The phase badge is padded to the widest phase in the current view; items without a phase show spaces when any visible item has a phase. If no visible items have a phase (`max_phase == 0`), the output uses `[]` with no interior spaces.
